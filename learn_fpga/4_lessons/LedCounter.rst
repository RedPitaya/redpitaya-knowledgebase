...
In this project, we will learn how to create a simple LED binary counter using basic VHDL code. We will also learn, how to replace one of the rarely used components in the Red Pitaya FPGA, the MIMO PID, and replace it with our own FPGA component. This way we can easily access the system bus to give programs access to registers and with a few small modifications gain access to ADCs and DACs, but let us not get ahead of ourselves. 
