...
where 'T' is the time constant of the filter. The higher the order of the filter, the more complex this equation becomes.
...
where 'h_d[n]' is the impulse response of an ideal filter, 'w[n]' is the window function, and 'n' ranges from 0 to N-1.
